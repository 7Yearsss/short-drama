...
面向台湾用户的付费短剧（短视频剧集）会员平台 MVP：LINE 登录（当前为开发期模拟）、免费集数 + 单剧解锁 + 月度会员三种付费墙组合、Cloudflare R2 视频存储 + 签名播放地址、本地 ffmpeg 转码上传工具、管理后台。
...
- **转码在本地机器完成，不占用 VPS 资源**：`tools/uploader` 是一个在开发者自己电脑上跑的 CLI，用 ffmpeg 转码后直接把成品上传到 R2，再调用管理 API 注册这一集的元数据。VPS 只跑轻量的 API + Postgres。
...
tools/
  uploader/           本地 CLI：ffmpeg 转码 → 上传 R2 → 调管理 API 注册集数
...
- 本地转码需要 `ffmpeg` / `ffprobe`（仅 `tools/uploader` 用到，跑 API/Web 不需要）
...
cp tools/uploader/.env.example tools/uploader/.env   # 仅在需要上传真实视频时配置
```
...
| `/admin/series/:id` | 集数管理：查看某剧全部集数状态、上架 |

内容发布流程：本地用 `tools/uploader` 转码+上传+注册（生成 `draft` 集数）→ 在 `/admin/series/:id` 点"上架"→ 立即在公开站可见。
...
| 视频编码 | 单一 MP4（H.264） | 如需自适应码率，只需改 `tools/uploader` 的 `runFfmpeg` 调用产出 HLS，`r2Key` 约定和播放接口都不用动 |
...
- **服务器规格**：API + Postgres 用 4 核 4G 的小型云主机即可（视频转码不在这台机器上跑，播放流量也直接打 R2，不经过这台机器）。
